...
export function detectLanguage(input: any): any {
    ...
    if (typeof input === "number") return NUMBER; // Handle numbers as number type
    if (typeof input === "boolean") return BOOLEAN; // Handle booleans as boolean type
    if (typeof input === "bigint") return NUMBER; // Handle bigints as bigint type
    ...
    for (const { name, country, regex } of unicodeMap) {
        if (regex.test(input)) return { name: name, country: country };
    }
